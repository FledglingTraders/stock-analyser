...
class TechnicalIndicators:
    @staticmethod
    def get_ohlcv_data(
        base_url: str, ticker: str, api_token: str, date_range: str = None
    ) -> List:
        # Fetch historical OHLCV data
        historical_url = (
            f"{base_url}/historical-price-full/{ticker}?apikey={api_token}{date_range}"
        )
    ...
    def get_tech_indicator_data(
        ...
        try:
            ...
            logger.error(
                f"Failed to fetch {indicator_type} data for {ticker}. HTTP Error: {http_error}"
            )
        except CustomException as error:
            logger.error(
                f"Failed to fetch {indicator_type} data for {ticker}. Error: {error}"
            )
        return []
